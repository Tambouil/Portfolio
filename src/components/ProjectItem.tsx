...
export const ProjectItem = memo(({ project, ...props }: ProjectItemProps) => {
  ...
  return (
    <a
      href={link}
      className={clsx('col-span-12 flex flex-col md:col-span-6 xl:col-span-4', props.className && props.className)}
    >
      <motion.article key={id} {...phoneMotionProps}>
        ...
        <AnimatedText text={area} className="mt-6 text-sm font-light text-dark-300 dark:text-dark-400" />
        <AnimatedText as="h3" text={name} className="mt-3 text-3xl text-dark-400 dark:text-dark-200" />
      </motion.article>
    </a>
  );
});
